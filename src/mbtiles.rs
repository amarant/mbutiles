...
fn parse_component(component: Component,
    ...
    if let Component::Normal(os_str) = component {
        let mut s = (try!(os_str.to_str()
                                .ok_or(MBTileError {
                                    message: format!("Unvalid unicode path: {:?}", os_str),
                                    inner_error: InnerError::None,
                                })))
                        .to_owned();
        match (flag_scheme, comp_type) {
            (Scheme::Ags, ComponentType::FirstDir) => {
                if !s.contains('L') {
                    warn!("You appear to be using an ags scheme on an non-arcgis Server cache.");
                }
                s = s.replace("L", "");
            }
            (Scheme::Ags, ComponentType::SecondDir(zoom)) => {
                s = s.replace("R", "");
                radix = 16;
            }
            (_, ComponentType::File(image_format, zoom)) => {
                let mut s_part: String; //espace E0506 for s
                {
                    let parts: Vec<&str> = s.split('.').collect();
                    let filtered_extension = get_extension(image_format);
                    if parts[1] == filtered_extension {
                        s_part = parts[0].to_owned();
                    } else {
            ...
            (_, ComponentType::FirstDir) => {
                if s.contains('L') {
                    warn!("You appear to be using a {} scheme on an arcgis Server cache. Try using --scheme=ags instead",
                          flag_scheme);
                }
            }
            (_, _) => (),
        }
        Ok(try_desc!(u32::from_str_radix(s.as_str(), radix),
